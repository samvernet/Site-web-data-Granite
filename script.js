...
function render(data) {
    ...
    grid.innerHTML = data.map((item, index) => `
        <div class="card" onclick="showFiche(${index})">
            ...
            <div class="status-pill">${item.Etat || 'ACTIF'}</div>
        </div>
    `).join('');
}
...
function showFiche(index) {
    ...
    document.getElementById('modalData').innerHTML = `
        ...
        <div class="info-row"><i class="fas fa-history"></i><div><label>Naissance & Décès</label><span>${item['Date de naissance'] || '?'} — ${item['Date de décés'] || '?'}</span></div></div>
